...
hash_table_file = "hash_table_cens_cosine.json"
...
def find_most_similar_song(input_features, hash_table):
    ...
    # Convert the input hash to a numpy array of integers
    input_hash_array = np.array([int(digit, 16) for digit in input_hash])

    # Normalize the input hash
    input_hash_array = input_hash_array / np.linalg.norm(input_hash_array)

    # Normalize the hash table
    normalized_hash_table = {key: (np.array([int(digit, 16) for digit in hash_table[key]]) / np.linalg.norm(np.array([int(digit, 16) for digit in hash_table[key]])))
                            if isinstance(hash_table[key], str) else hash_table[key] for key in hash_table}

    # Compare the input hash with the hashes in the hash table using cosine similarity
    similarity_scores = {key: 1 - cosine(normalized_hash_table[key].flatten(), input_hash_array.flatten()) for key in normalized_hash_table}
...
def plot_similarity(similarity_scores):
    ...
    plt.title('Cosine Similarity Scores with Different Songs')
